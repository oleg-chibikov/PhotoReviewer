...
public sealed class WindowsArranger(WindowFactory<IMainWindow> mainWindowFactory, ILogger<WindowsArranger> logger)
{
    ...
    readonly WindowFactory<IMainWindow> _mainWindowFactory = mainWindowFactory ?? throw new ArgumentNullException(nameof(mainWindowFactory));

    public IEnumerable<IPhoto> Photos => PhotoWindows.Select(x => x.Photo);

    List<IPhotoWindow> PhotoWindows { get; } = new List<IPhotoWindow>();
